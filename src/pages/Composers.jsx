      ...
      try {
        const response = await fetch(
          `https://corsproxy.io/?https://imslp.org/api.php?action=query&list=categorymembers&cmtitle=Category:Composers&cmstartsortkeyprefix=${encodeURIComponent(
            selectedLetter
          )}&cmlimit=1000&format=json&origin=*`
        );
        if (!response.ok) {
...
// Spinner beim Laden der Daten
  ...
  return (
    <div className="p-4">
      <h2 className="text-2xl font-bold mb-4">Liste der Komponisten</h2>
      ...
      <div className="flex flex-wrap justify-center mb-6">
        {letters.map((letter) => (
          <button
            ...
            className={`px-3 py-2 m-1 rounded-full transition-colors duration-300 ${
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {composers.map((composer) => (
          <li key={composer.pageid} className="bg-white rounded-lg shadow-sm p-4 hover:shadow-xl transition-shadow duration-300">
            <Link
              to={`/composer/${encodeURIComponent(composer.title.replace(/^Category:/, '').trim())}`}
              target="_blank"
